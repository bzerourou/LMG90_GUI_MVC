...
class ContactTab(BaseTab):
    ...
    def load_for_edit(self, law: ContactLaw):
        ...
        if 's1'        in p: self.s1_input.setText(str(p['s1']))     
        if 's2'        in p: self.s2_input.setText(str(p['s2']))    
        if 'G1'        in p: self.g1_input.setText(str(p['G1']))       
        if 'G2'        in p: self.g2_input.setText(str(p['G2']))
        ...
        if 'Fmax'      in p: self.sigc_input.setText(str(p['Fmax']))
        if 'rstn'      in p: self.rstn_input.setText(str(p['rstn']))
        if 'rstt'      in p: self.rstt_input.setText(str(p['rstt']))
        if 'kn'        in p: self.kn_input.setText(str(p['kn']))
        if 'stiffness' in p and law.law_type.value == ContactLawType.ELASTIC_REPELL_CLB:
            self.kn_input.setText(str(p['stiffness']))
